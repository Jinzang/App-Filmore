...
sub populate_object {
    ...
    foreach my $field (keys %parameters) {
        if (exists $configuration->{$field}) {
			my $ref = ref $parameters{$field};

			if ($field =~ /_ptr$/) {
				$self->{$field} = $configuration->{$field};
			} elsif ($ref eq ref $configuration->{$field}) {
                $self->{$field} = $configuration->{$field};
            } elsif ($ref eq 'ARRAY' && ! ref $configuration->{$field}) {
                $self->{$field} = [$configuration->{$field}];
            } else {
                $ref ||= 'scalar';
                die "Configuration file field $field is not a $ref\n";
            }
